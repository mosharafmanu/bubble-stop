...
?>
	...
	<main id="primary" class="site-main blog-archive-page">
		...
		<header class="archive-header layout-padding pt-50 pt-md-70 pt-lg-100">
			<?php
			if ( is_category() ) {
				$archive_label = __( 'Category', 'bubble-stop' );
				$archive_title = single_cat_title( '', false );
			} elseif ( is_tag() ) {
				$archive_label = __( 'Tag', 'bubble-stop' );
				$archive_title = single_tag_title( '', false );
			} elseif ( is_author() ) {
				$archive_label = __( 'Author', 'bubble-stop' );
				$archive_title = esc_html( get_the_author() );
			} elseif ( is_year() ) {
				$archive_label = __( 'Year', 'bubble-stop' );
				$archive_title = get_the_date( 'Y' );
			} elseif ( is_month() ) {
				$archive_label = __( 'Month', 'bubble-stop' );
				$archive_title = get_the_date( 'F Y' );
			} elseif ( is_day() ) {
				$archive_label = __( 'Date', 'bubble-stop' );
