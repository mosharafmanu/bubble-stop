...
if ( ! function_exists( 'bubble_stop_render_video' ) ) {
	function bubble_stop_render_video( $video_field, $args = [] ) {
		$defaults = [
			'behavior'           => 'autoplay',
			'autoplay'           => true,
		...
		switch ( $video_source ) {
			case 'self_host':
				$video_html = bubble_stop_render_self_hosted_video( $video_data, $args );
				break;
			case 'youtube':
				$video_html = bubble_stop_render_youtube_video( $video_data, $args );
				break;
			case 'vimeo':
				$video_html = bubble_stop_render_vimeo_video( $video_data, $args );
				break;
			case 'cdn':
				$video_html = bubble_stop_render_cdn_video( $video_data, $args );
				break;
		}
		...
		if ( 'onclick-popup' === $args['behavior'] ) {
			...
			$html .= '<button class="video-play-button" aria-label="' . esc_attr__( 'Play Video', 'bubble-stop' ) . '">';
		...
		if ( 'autoplay' === $args['behavior'] && $args['controls'] && 'youtube' !== $video_source ) {
			if ( $args['autoplay_on_scroll'] || $args['autoplay'] ) {
				...
				$html .= '<button class="video-play-button low-power-play-btn" aria-label="' . esc_attr__( 'Play Video', 'bubble-stop' ) . '">';
				$html .= '<svg xmlns="http://www.w3.org/2000/svg" width="22" height="26" viewBox="0 0 22 26" fill="none">';
			...
			if ( function_exists( 'bubble_stop_render_video_autoplay_controls' ) ) {
				$html .= bubble_stop_render_video_autoplay_controls( [ 'echo' => false ] );
			}
		}
...
if ( ! function_exists( 'bubble_stop_render_self_hosted_video' ) ) {
	function bubble_stop_render_self_hosted_video( $video_data, $args ) {
		...
		$html .= esc_html__( 'Your browser does not support the video tag.', 'bubble-stop' );
		$html .= '</video>';
...
if ( ! function_exists( 'bubble_stop_render_youtube_video' ) ) {
	function bubble_stop_render_youtube_video( $video_data, $args ) {
		...
		if ( 'onclick-popup' === $args['behavior'] ) {
			...
			$html = '<img src="' . esc_url( $thumbnail_url ) . '" alt="' . esc_attr__( 'Video Thumbnail', 'bubble-stop' ) . '" ';
...
if ( ! function_exists( 'bubble_stop_render_vimeo_video' ) ) {
	function bubble_stop_render_vimeo_video( $video_data, $args ) {
...
if ( ! function_exists( 'bubble_stop_render_cdn_video' ) ) {
	function bubble_stop_render_cdn_video( $video_data, $args ) {
