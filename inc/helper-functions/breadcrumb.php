...
if ( ! function_exists( 'bubble_stop_breadcrumb' ) ) {
	function bubble_stop_breadcrumb( $layout_padding = false, $margin_top = 'mt-30', $margin_bottom = '' ) {
		if ( is_front_page() ) {
			return;
		}

		$classes = [ 'bubble-stop-breadcrumb' ];
		...
		echo '<a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'bubble-stop' ) . '</a>';
		...
		if ( is_home() ) {
			...
			if ( $posts_page ) {
				...
				echo '<span class="current">' . esc_html__( 'Blog', 'bubble-stop' ) . '</span>';
			}
			...
			if ( $posts_page ) {
				...
				echo '<a href="' . esc_url( home_url( '/blog/' ) ) . '">' . esc_html__( 'Blog', 'bubble-stop' ) . '</a>';
			}
			echo ' <span class="breadcrumb-separator">/</span> ';
			...
			echo '<span class="current">' . sprintf( esc_html__( 'Search results for: %s', 'bubble-stop' ), get_search_query() ) . '</span>';

		} elseif ( is_404() ) {
			echo '<span class="current">' . esc_html__( '404 Not Found', 'bubble-stop' ) . '</span>';
