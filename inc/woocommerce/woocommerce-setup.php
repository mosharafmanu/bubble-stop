<?php
/**
 * Bubble Stop — WooCommerce Module
 *
...
// (bubble-stop-woocommerce.css) places each as its own auto-placed grid
...
// terms (`.account-auth` in bubble-stop-woocommerce.css), independent of
...
add_action( 'wp_enqueue_scripts', function() {
	...
	wp_enqueue_style(
		...
		get_template_directory_uri() . '/assets/css/woocommerce/bubble-stop-woocommerce.css',
		[ 'bubble-stop-starter-style' ],
		BUBBLE_STOP_VERSION
	);
	...
	// runs). Kept independent of bubble-stop-woocommerce so it stays
	// intact even when other WC templates are themed per project.
	wp_enqueue_style(
		'bubble-stop-notices',
		get_template_directory_uri() . '/assets/css/woocommerce/bubble-stop-notices.css',
		[ 'bubble-stop-starter-style' ],
		BUBBLE_STOP_VERSION
	);
	...
	// Kept independent of bubble-stop-woocommerce so it stays intact
	// even when other WC templates are themed per project.
	wp_enqueue_style(
		'bubble-stop-quantity',
		get_template_directory_uri() . '/assets/css/woocommerce/bubble-stop-quantity.css',
		[ 'bubble-stop-starter-style' ],
		BUBBLE_STOP_VERSION
	);
	wp_enqueue_script(
		'bubble-stop-quantity',
		get_template_directory_uri() . '/assets/js/woocommerce/bubble-stop-quantity.js',
		[ 'jquery' ],
		BUBBLE_STOP_VERSION,
		true
	);
	...
	// Kept independent of bubble-stop-quantity and bubble-stop-woocommerce
	...
	wp_enqueue_style(
		'bubble-stop-cart-instant-update',
		get_template_directory_uri() . '/assets/css/woocommerce/bubble-stop-cart-instant-update.css',
		[ 'bubble-stop-starter-style' ],
		BUBBLE_STOP_VERSION
	);
	wp_enqueue_script(
		'bubble-stop-cart-instant-update',
		get_template_directory_uri() . '/assets/js/woocommerce/bubble-stop-cart-instant-update.js',
		[ 'jquery' ],
		BUBBLE_STOP_VERSION,
		true
	);
	...
	if ( is_shop() || is_product_taxonomy() ) {
		wp_enqueue_style(
			...
			get_template_directory_uri() . '/assets/css/woocommerce/bubble-stop-shop-archive.css',
			[ 'bubble-stop-woocommerce' ],
			BUBBLE_STOP_VERSION
		);
		wp_enqueue_script(
