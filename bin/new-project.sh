#!/usr/bin/env bash
# =============================================================
#  Bubble Stop — New Project Bootstrap
#
#  Run from the theme root AFTER copying bubble-stop into
...
# =============================================================
echo ""
echo -e "${BOLD}  Bubble Stop — New Project Bootstrap${RESET}"
...
if [[ "$THEME_SLUG" == "bubble-stop" ]]; then
    die "Theme Slug cannot be 'bubble-stop'. Copy the folder first, then run this script inside the copy."
fi

# Derived: PHP constant prefix — uppercase slug, hyphens → underscores
# (e.g. nexora → NEXORA, travelnerds-theme → TRAVELNERDS_THEME).
# This is how BUBBLE_STOP_VERSION itself was built from bubble-stop.
...
echo ""
printf "  %-28s →  %s\n" "Bubble Stop"        "$THEME_NAME"
printf "  %-28s →  %s\n" "bubble-stop"        "$THEME_SLUG"
printf "  %-28s →  %s\n" "BUBBLE_STOP_"       "${CONST_PREFIX}_"
printf "  %-28s →  %s\n" "bubble_stop_"       "${PHP_PREFIX}_"
printf "  %-28s →  %s\n" "--bubb-"            "--${CSS_PREFIX}-"
printf "  %-28s →  %s\n" "bg-bubb-  color-bubb-" "bg-${CSS_PREFIX}-  color-${CSS_PREFIX}-"
printf "  %-28s →  %s\n" "'bubb-N'  \"bubb-N\""  "'${CSS_PREFIX}-N'  \"${CSS_PREFIX}-N\""
...
# 1. Display name — must come BEFORE slug replacement.
n=$(replace_in_files "Bubble Stop" "$THEME_NAME" "${ALL_FILES[@]}")
ok "Display name 'Bubble Stop' → '$THEME_NAME'  ($n files)"
CHANGED=$((CHANGED + n))

# 2. Slug — covers text domain, enqueue handles, URLs, ACF JSON keys
n=$(replace_in_files "bubble-stop" "$THEME_SLUG" "${ALL_FILES[@]}")
ok "Slug 'bubble-stop' → '$THEME_SLUG'  ($n files)"
CHANGED=$((CHANGED + n))

# 3. PHP constant prefix — e.g. BUBBLE_STOP_VERSION → NEXORA_VERSION
n=$(replace_in_files "BUBBLE_STOP_" "${CONST_PREFIX}_" "${ALL_FILES[@]}")
ok "PHP constant prefix 'BUBBLE_STOP_' → '${CONST_PREFIX}_'  ($n files)"
CHANGED=$((CHANGED + n))

# 4. PHP function prefix
n=$(replace_in_files "bubble_stop_" "${PHP_PREFIX}_" "${ALL_FILES[@]}")
ok "PHP prefix 'bubble_stop_' → '${PHP_PREFIX}_'  ($n files)"
CHANGED=$((CHANGED + n))

# 5. CSS custom property prefix
n=$(replace_in_files "--bubb-" "--${CSS_PREFIX}-" "${ALL_FILES[@]}")
ok "CSS var prefix '--bubb-' → '--${CSS_PREFIX}-'  ($n files)"
CHANGED=$((CHANGED + n))

# 6. CSS background utility classes
n=$(replace_in_files "bg-bubb-" "bg-${CSS_PREFIX}-" "${ALL_FILES[@]}")
ok "CSS class 'bg-bubb-' → 'bg-${CSS_PREFIX}-'  ($n files)"
CHANGED=$((CHANGED + n))

# 7. CSS text colour utility classes
n=$(replace_in_files "color-bubb-" "color-${CSS_PREFIX}-" "${ALL_FILES[@]}")
ok "CSS class 'color-bubb-' → 'color-${CSS_PREFIX}-'  ($n files)"
CHANGED=$((CHANGED + n))

# 8. PHP image-size slug references (single-quoted strings)
n=$(replace_in_files "'bubb-" "'${CSS_PREFIX}-" "${PHP_FILES[@]}")
ok "PHP image sizes 'bubb-N' → '${CSS_PREFIX}-N'  ($n files)"
CHANGED=$((CHANGED + n))

# 9. PHP image-size slug references (double-quoted strings)
n=$(replace_in_files "\"bubb-" "\"${CSS_PREFIX}-" "${PHP_FILES[@]}")
[[ $n -gt 0 ]] && ok "PHP image sizes \"bubb-N\" → \"${CSS_PREFIX}-N\"  ($n files)" \
    || skipped "Double-quoted bubb- image sizes: none found"
CHANGED=$((CHANGED + n))

# Rename files whose names contain the starter slug. Replacing file contents
# alone leaves enqueue URLs pointing at files that do not exist.
step "Renaming slug-bearing files"
RENAMED=0
while IFS= read -r old_path; do
    old_dir="${old_path%/*}"
    old_name="${old_path##*/}"
    new_name="${old_name//bubble-stop/$THEME_SLUG}"
    new_path="$old_dir/$new_name"
    if [[ -e "$new_path" ]]; then
        warn "Cannot rename ${old_path#"$THEME_DIR"/}: target already exists"
        continue
    fi
    mv "$old_path" "$new_path"
    ok "Renamed ${old_path#"$THEME_DIR"/} → ${new_path#"$THEME_DIR"/}"
    RENAMED=$((RENAMED + 1))
done < <(find "$THEME_DIR" \
    -not -path "*/node_modules/*" \
    -not -path "*/vendor/*" \
    -not -path "*/.git/*" \
    -not -path "*/bin/new-project.sh" \
    -type f -name '*bubble-stop*')
CHANGED=$((CHANGED + RENAMED))
[[ $RENAMED -eq 0 ]] && skipped "No slug-bearing filenames found"
...
if [[ -f "$README" ]]; then
    # Global replacements already handled bubble_stop_ and bubble-stop.
    # Update the H1 title if it still contains the old name (edge case).
    if grep -q "^# Bubble Stop\b" "$README" 2>/dev/null; then
        perl -pi -e "s/^# Bubble Stop\b/# $THEME_NAME/" "$README"
...
OLD_POT="$LANG_DIR/bubble-stop.pot"
NEW_POT="$LANG_DIR/${TEXT_DOMAIN}.pot"

if [[ -f "$OLD_POT" ]]; then
    mv "$OLD_POT" "$NEW_POT"
    ok "Renamed: bubble-stop.pot → ${TEXT_DOMAIN}.pot"
    CHANGED=$((CHANGED + 1))
elif [[ -f "$NEW_POT" ]]; then
...
REMAINING=$( \
    { grep -rl "bubble-stop\|bubble_stop_\|Bubble Stop\|--bubb-\|bg-bubb-\|color-bubb-" \
        "$THEME_DIR" \
        --include="*.php" --include="*.css" --include="*.js" \
        --include="*.json" --include="*.md" \
        2>/dev/null || true; } \
    | { grep -v "node_modules\|vendor\|\.git\|bin/new-project.sh" || true; } \
    | wc -l | tr -d ' ' \
)
