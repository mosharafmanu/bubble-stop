# Bubble Stop — WooCommerce Integration
...
| Our WC CSS enqueued | `bubble-stop-woocommerce.css`, after `bubble-stop-starter-style` |
...
### CSS (`assets/css/woocommerce/bubble-stop-woocommerce.css`)

Fully replaces WC default styles. Uses only `var(--bubb-*)` tokens. Sections:
...
| CSS | `assets/css/woocommerce/bubble-stop-shop-archive.css` |
| JS | `assets/js/woocommerce/bubble-stop-shop-archive.js` |
| Icons | `assets/svgs/filter-icon.php`, `close-icon.php` |

**To remove** (leaving the stock WooCommerce archive layout, still themed by
`bubble-stop-woocommerce.css`): delete the require line for `shop-archive.php` in
`woocommerce-setup.php`, the hooks file itself, the three `shop-*.php` templates, and the
`bubble-stop-shop-archive` CSS/JS pair.
...
- **Result count**: relocated from `.result-count-wrapper` to `.result-count-wrapper-mobile`
  below ≤1199px by `bubble-stop-shop-archive.js`.
- **Pagination**: WC's default is removed; `bubble_stop_render_pagination()` is used instead, so
  shop pagination matches blog/archive pagination styling.
- **Flexible content**: `bubble_stop_load_shop_flexible_content()` prints the `cms` field below the
...
- **Sidebar width / breakpoints / colours**: all in `bubble-stop-shop-archive.css`, using only
  `var(--bubb-*)` tokens — adjust per the project's design grid.
- **Filter taxonomy**: hardcoded to `product_cat`. To filter by a different taxonomy (brand,
  attribute, etc.), adapt `bubble_stop_filter_shop_products_by_category()` and `shop-filters.php`.
...
4. **Review the color tokens** — the WC CSS inherits everything from `--bubb-*`. No hex values should need changing unless the project design diverges from the tokens.
...
The dispatcher (`bubble_stop_flexible_content('cms')`) can be called from a section within the single product template if a project needs flexible layouts on product pages. Add it after the WC product data section or in a custom tab.
