# Bubble Stop — New Project Setup Script
...
Copy `bubble-stop/` to your new project's WordPress themes directory and rename it to your project slug:

```bash
# Example
cp -r /path/to/bubble-stop /your/wp/wp-content/themes/travelnerds-theme
```

> Do **not** run the script inside the `bubble-stop` folder itself. Always work on a copy.
...
> A copy like `wp-content/themes/my-project/bubble-stop/style.css` (one level too
...
> ```bash
> mv /your/wp/wp-content/themes/my-project/bubble-stop /your/wp/wp-content/themes/travelnerds-theme
...
> (`mosharafmanu/bubble-stop`). Committing or pushing from the copy without
...
git init
git add .
git commit -m "Initial copy from bubble-stop starter"
```
...
| PHP Function Prefix | `travelnerds` | All `bubble_stop_` → `travelnerds_` function names |
| CSS Token Prefix | `tn` | All `--bubb-` → `--tn-` CSS custom properties and utility classes |
...
| From | To |
|---|---|
| `Bubble Stop` | Theme Name |
| `bubble-stop` | Theme Slug |
| `bubble_stop_` | `{prefix}_` |
| `--bubb-` | `--{css-prefix}-` |
| `bg-bubb-` | `bg-{css-prefix}-` |
| `color-bubb-` | `color-{css-prefix}-` |
| `'bubb-N'` / `"bubb-N"` | `'{css-prefix}-N'` (image size slugs) |

### Specific file updates

| File | What changes |
|---|---|
| `style.css` `:root {}` | Token prefix via global `--bubb-` replacement |
...
| `assets/**/*bubble-stop*` | Filename slug renamed to the project slug so enqueue URLs keep matching |
| `languages/bubble-stop.pot` | Renamed to `languages/{text-domain}.pot` |
| `.ai/*.md` | All `bubble_stop_`, `bubble-stop`, `--bubb-` references |
...
**Script says Theme Slug cannot be 'bubble-stop'**
...
```bash
perl -pi -e 's/bubble-stop/your-slug/g' path/to/the/file.php
```
...
git checkout .
```
If you didn't, copy the original `bubble-stop` starter again and start fresh.
