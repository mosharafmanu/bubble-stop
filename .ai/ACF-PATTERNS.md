# Bubble Stop — ACF Patterns & Helper Functions

Bubble Stop is a clean ACF-based WordPress theme framework. Every section is created per project from the client design.
...
bubble_stop_flexible_content('cms', $post_id);
```
...
// ✅ Guard helper calls
if ( $button && function_exists( 'bubble_stop_render_button' ) ) {
    bubble_stop_render_button( $button );
}
...
```php
bubble_stop_flexible_content( string $field_name = 'cms', int|null $post_id = null )
```

```php
bubble_stop_get_first_flexible_layout() : string|false
bubble_stop_get_last_flexible_layout()  : string|false
bubble_stop_has_hero_first_section()    : bool
```

Global variables available inside section templates during render:
```php
$GLOBALS['bubble_stop_previous_layout']   // name of previous layout (string)
$GLOBALS['bubble_stop_section_index']     // 0-based position on the page (int)
$GLOBALS['bubble_stop_last_layout']       // name of last layout on the page (string)
```

Use for spacing/padding logic:
```php
if ( 0 === $GLOBALS['bubble_stop_section_index'] ) {
...
```php
bubble_stop_render_responsive_picture( array $image, array $args = [] )
```
...
bubble_stop_render_responsive_picture( $image, [
    'size_group'     => 'hero',
    'lazy'           => false,
    'fetchpriority'  => 'high',
    'class'          => 'hero-image',
...
```php
bubble_stop_render_button( array $button_link, array $args = [] )
```
...
```php
bubble_stop_render_buttons( array $buttons, array $args = [] )
```
...
```php
bubble_stop_render_icon( array|int $icon, array $args = [] )
```
...
```php
bubble_stop_render_video( string|array $video_field, array $args = [] )
```
...
```php
// Header — logo
bubble_stop_get_site_logo()           : array|false
bubble_stop_render_site_logo()        : void

// Header — CTA button
bubble_stop_get_header_button()       : array|false
bubble_stop_render_header_button()    : void

// Footer — logo (falls back to site_logo if footer_logo not set)
bubble_stop_get_footer_logo()         : array|false
bubble_stop_render_footer_logo()      : void

// Footer — tagline (short description below logo, not rendered by default)
bubble_stop_get_footer_tagline()      : string|false
bubble_stop_render_footer_tagline()   : void

// Footer — social icons (repeater: social_icon + social_link)
bubble_stop_get_social_medias()       : array|false
bubble_stop_render_social_medias()    : void

// Footer — copyright text (supports {year} placeholder)
bubble_stop_get_footer_copyright()    : string|false
bubble_stop_render_footer_copyright() : void
```

#### Footer menu helper

```php
bubble_stop_render_footer_menu( array $args = [] ) : void
```
...
```php
bubble_stop_render_footer_menu( [ 'location' => 'footerMenu', 'show_title' => false ] );
```
...
// footer.php
bubble_stop_render_footer_menu( [ 'location' => 'footerMenu',  'show_title' => true ] );
bubble_stop_render_footer_menu( [ 'location' => 'footerMenu2', 'show_title' => true ] );
bubble_stop_render_footer_menu( [ 'location' => 'footerMenu3', 'show_title' => true ] );
```

#### Starter footer structure

```
footer-top   (background: --bubb-color-primary)
  logo                              footer menu (flat, no title)

footer-bottom  (background: --bubb-color-secondary)
...
```php
bubble_stop_breadcrumb( bool $layout_padding = false, string $margin_top = 'mt-30', string $margin_bottom = '' )
```
...
```php
bubble_stop_render_pagination( WP_Query $query = null )
```
...
$section_classes = [ 'my-section', 'bg-bubb-' . $bg ];
```
...
Grid layout classes are defined globally in `assets/css/bubble-stop-theme-style.css`. **Never redefine grid columns inside a section's own CSS.** Use these classes directly in section templates.
...
Use `bubble_stop_render_post_card()` inside a `.card-grid.columns-3`. Pass `fetchpriority => 'high'` for the first card (above the fold) and `variant => 'featured'` to mark it for project-specific featured styling:
...
<div class="blog-grid card-grid columns-3">
    ...
    while ( have_posts() ) :
        the_post();
        $i++;
        bubble_stop_render_post_card( null, [
...
| Field | Type | Helper |
|---|---|---|
| `site_logo` | Image | `bubble_stop_get_site_logo()` / `bubble_stop_render_site_logo()` |
| `header_button` | Link | `bubble_stop_get_header_button()` / `bubble_stop_render_header_button()` |
| `header_button_mobile_phone` | Text | `bubble_stop_get_header_button_mobile_phone()` |
| `footer_logo` | Image | `bubble_stop_get_footer_logo()` / `bubble_stop_render_footer_logo()` |
| `social_medias` | Repeater (social_icon + social_link) | `bubble_stop_get_social_medias()` / `bubble_stop_render_social_medias()` |
| `footer_copyright` | Text | `bubble_stop_get_footer_copyright()` / `bubble_stop_render_footer_copyright()` |
