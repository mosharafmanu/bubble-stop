# Bubble Stop — Content Typography & Single Post
...
**CSS file:** `assets/css/bubble-stop-design-style.css` — "CONTENT TYPOGRAPHY" section.
...
<article class="single-post">                            ← post_class() adds WP classes
  <div class="post-thumbnail">                           ← full-width, max-height: 34rem (18rem mobile)
    <picture> / <img class="post-thumbnail-image">       ← bubble_stop_render_responsive_picture(), LCP
  </div>
...
**`assets/css/bubble-stop-starter-style.css` — "Single Post" section:**
...
The featured image uses `bubble_stop_render_responsive_picture()` with `fetchpriority: 'high'` and `lazy: false` — it is always above the fold on a single post. Falls back to `the_post_thumbnail('mc-1200')` if the helper is unavailable.
